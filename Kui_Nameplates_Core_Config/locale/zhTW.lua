...
opt.tooltips["absorb_enable"] = "在血條上顯示吸收層"
opt.tooltips["absorb_striped"] = "在吸收層使用條紋材質。如果未勾選，繼承血條材質"
opt.tooltips["auras_centre"] = "水平置中對齊框架上的圖標，而非從左側"
opt.tooltips["auras_enabled"] = "顯示你施放在血條上的光環 - 友方為增益，敵對為減益"
opt.tooltips["auras_icon_minus_size"] = "小框架的圖標大小"
opt.tooltips["auras_icon_normal_size"] = "一般尺寸框架的圖標大小"
opt.tooltips["auras_icon_squareness"] = "光環圖標的長寬比例，當為1代表是正方形"
opt.tooltips["auras_maximum_length"] = "不顯示初始長度大於此值的光環。設定 -1 為忽"
opt.tooltips["auras_minimum_length"] = "不顯示初始長度少於此值的光環"
opt.tooltips["auras_on_personal"] = "如果啟用顯示你角色血條的光環"
opt.tooltips["auras_pulsate"] = "當快要過期時閃耀圖標"
opt.tooltips["auras_time_threshold"] = "當光環的剩餘時間少於這個數值時顯示倒數文字。設定為 -1 會永遠顯示此計時"
opt.tooltips["auras_vanilla_filter"] = "在敵對血條使用預設UI的光環過濾，顯示諸如其他玩家的東西\\' 暈、沉默、等等., 與某些重要的玩家法術。|n|n|cffff6666目前沒有辦法編輯此過濾器包含的法術。|n|n注意啟用此功能時敵對的KSLC白名單是被忽略的。"
opt.tooltips["auras_whitelist"] = "使用KuiSpellList來過濾法術。你可以使用在Curse上的額外KuiSpellListConfig插件來自訂此清單。|n|n|cffff6666如果\"使用預設UI過濾器\"也啟用，則此選項和白名單將被忽略。"
